...
const ProjectModal = ({ data, isOpen, setIsOpen }) => {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-end justify-end transition-opacity duration-[500ms] ease-in-out ${isOpen ? "md:bg-black md:bg-opacity-50" : "pointer-events-none"
        }`}
      onClick={handleClose}
    >
      <div
        className={`scrollBar z-10 h-full w-screen overflow-y-auto border-l-[5px] border-solid border-pink-global bg-off-white p-4 shadow-lg transition-transform duration-[800ms] ease-in-out md:w-2/3 lg:w-1/2 ${isOpen ? "translate-x-0" : "translate-x-full"
          }`}
      ...
      >
        ...
        <div>
          ...
          <p className="mt-4 text-[0.940rem] leading-[1.6rem] md:text-base">
            ...
            {/* Affichage de l'aperçu sous forme de liste à puces */}
